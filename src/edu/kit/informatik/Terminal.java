...
terminal as well as a method to read in
* files.
*
* <p><b>Never modify this class, never upload it to Praktomat.</b> This is only
for your local use. If an assignment
* tells you to use this class for input and output never use System.out,
System.err or System.in in the same
* assignment.
*
...
* of characters, arrays, and lines. This stream is already open and ready to
supply input data and corresponds
* to keyboard input.
*/
private static final BufferedReader IN = new BufferedReader(new
...
*
* <p>More specific, this method behaves exactly as if the following code got
executed:
* <blockquote><pre>
...
the line.
*
* <p>If the argument is {@code null}, then the string {@code "null"} is
printed, otherwise the object's string
...
*
* <p>If the argument is {@code null}, then a {@code NullPointerException} is
...
behaves exactly as
* System.out regarding output. (System.out.println(char[]) calls
String.valueOf(char[])
* which itself returns 'new String(char[])' and is therefore the only
method that behaves
* differently when passing the provided parameter to the
System.out.println(Object)
* method.)
*/
...
{@code String} array, whereas the
* first array field contains the file's first line, the second field
...
/*
* You can expect that the praktomat exclusively provides valid filepaths.
Therefore
* there will no IOException occur while reading in files during the
tests, the
* following RuntimeException does not have to get handled.
*/
throw new RuntimeException(e);
